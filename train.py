...
def run_batch(model, epoch, batch, optimizer, df, stage, p_bar):
	out, loss = model.train_step(batch)
	loss.backward()
	optimizer.step()
	optimizer.zero_grad()
	score = f_score(out, batch[1])
	results = {"loss": loss.detach().item(), "f_score": score.item()}
...
def trainining_pipeline(experiment_name="base"):
	...
	if not log_dir.joinpath("model_last.pth").exists():
		for epoch in range(settings.EPOCHS):
			...
			for batch in p_bar:
				model, log_df = run_batch(model, epoch, batch, optimizer, log_df, "train", p_bar)

			model.eval()
			p_bar = tqdm(val_loader, desc="Validation")
			for batch in tqdm(val_loader):
				model, log_df = run_batch(model, epoch, batch, optimizer, log_df, "valid", p_bar)
